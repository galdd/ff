...
/**
 * Created by gald2 on 2/1/2016.
 */
public class RootViewClasic extends ViewGroup {
private View mMenu;// menu //left view
private View mHost;// host // main screen
private int mMenuId;// menuid
private int mHostId;// hostid // on FrameLayout on the Xml
private int mHostRemainWidth;// After the menu opens ghost retains width
...
private boolean mGestureToRight = false;// slide to right
...
public interface OnAnimationComplete {
    ...
    public RootViewClasic(Context context, AttributeSet attrs, int defStyle) {
    ...
    public RootViewClasic(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RootViewClasic(Context context) {
        super(context);
        init(context);
    }
                ...
                if (mActivePointerId != INVALID_POINTER_ID) {
                    ...
                    if (isDragging()) {
                        ...
                        if (right + distance < mHostWidth) {
                            // Fixed host session left off screen range
                            distance = mHostWidth - right;
                        }
                        int left = mHost.getLeft();
                        if (left + distance > mMenuWidth) {
                            // Over menu right border width correction menu
                            distance = mMenuWidth - left;
                        }

                        // Record final gesture direction
                        if (distance > 0) {
                            // user want open
...
class Scrolling implements Runnable {
    ...
    public void run() {
        ...
        if (more) {
            ...
            if (diff != 0) {
                if (isDragging()) {
                    // If the current view is being dragged, then let go when the user resets the drag state
                    mState ^= MENU_DRAGGING;
                }
                if (!isFlinging()) {
    ...
    public LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new RootViewClasic.LayoutParams(getContext(), attrs);
    }
